...
export default function OrderHistory({ onReorder }: { onReorder: () => void }) {
  ...
  function handleReorder(order: Order) {
    ...
    if (added.length > 0) {
      let msg = `${added.length} item${added.length > 1 ? "s" : ""} added toCart.`;
          ...
          <div key={order.id} className={cn(
            ...
            <div className="flex items-start justify-between px-4 pt-4 pb-3 border-b border-border">
              ...
              <div className="flex flex-col items-end gap-2">
                <span className={cn("px-2.5 py-1 rounded-full text-[10px] font-bold border flex items-center gap-1", ORDER_STATUS_CLASS[order.status])}>
            ...
            <div className="px-4 pb-4 flex items-center justify-between gap-2">
              <span className="text-[11px] text-muted-foreground capitalize">
                {order.paymentMethod === "cash" ? "Cash on Delivery" : order.paymentMethod.toUpperCase()}
              </span>
              <div className="flex items-center gap-2">
                {/* Track active orders */}
                {isActive && (
