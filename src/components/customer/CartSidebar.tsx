      ...
      {!open ? (
        ...
        <div className="space-y-1">
          <input
            ...
            placeholder="e.g. Extra sauce, Less spicy..."className="w-full text-xs border border-amber-300 rounded-lg px-2 py-1 bg-amber-50 focus:outline-none focus:ring-1 focus:ring-amber-400"
          />
...
export default function CartSidebar({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  function handleItemNote(menuItemId: string, note: string) {
    ...
    if (!item) return;
    // Update note viaCart item update — store note inCartItem
    dispatch({
  ...
  // Diagram 1: Order saved as "pending", cashier confirms it
  async function placeOrder() {
    ...
    if (user) {
      ...
      } catch (err) { console.error("[CartSidebar] saveOrder:", err); }
      if (user.email) {
      ...
      <div className="relative ml-auto w-full max-w-md bg-white h-full flex flex-col shadow-2xl">
          ...
          {step === "delivery" && (
            <div className="p-4 space-y-4">
              ...
              <div className="space-y-1.5">
                ...
                <Textarea placeholder="Any notes for the kitchen or rider..." value={notes} onChange={e => setNotes(e.target.value)} rows={3} />
              </div>
            </div>
          )}
        ...
        {(cart.items.length > 0 || step === "receipt") && (
          <div className="p-4 border-t border-border space-y-3 bg-white">
            ...
            {step === "delivery"&& <Button className="w-full" onClick={() => setStep("payment")} disabled={!name || !phone || (cart.orderType === "delivery" && !address)}>Continue to Payment</Button>}
            {step === "payment" && <Button className="w-full" onClick={() => setStep("confirm")}>Review Order</Button>}
