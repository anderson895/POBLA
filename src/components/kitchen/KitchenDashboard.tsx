...
export default function KitchenDashboard() {
  ...
  async function handleConfirm(id: string) { await updateOrderStatus(id, "confirmed"); }
  async function handleCancel(id: string)  { await updateOrderStatus(id, "cancelled"); }
  ...
  const counts = {
    ...
    confirmed: kitchen.filter(o => o.status === "confirmed").length,
            ...
            <>
              ...
              {kitchenFiltered.length === 0 ? (
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                  {kitchenFiltered
                    .sort((a, b) =>
                      (({ confirmed:0, preparing:1, ready:2 } as Record<string,number>)[a.status] ?? 3) -
                      (({ confirmed:0, preparing:1, ready:2 } as Record<string,number>)[b.status] ?? 3)
                    )
