...
const ROLE_META: Record<UserRole, { label: string; icon: React.ReactNode }> = {
  ...
  staff:    { label: "Staff",           icon: <WrenchScrewdriverIcon className="w-4 h-4" /> },
};
    ...
    >
      <div className="max-w-7xl mx-auto px-4 h-16 flex items-center justify-between gap-4">
        ...
        <div
          ...
          style={{ background: "rgba(188,93,93,0.12)", border: "1px solid rgba(188,93,93,0.3)" }}
        >
          <span className="text-brand">{roleMeta.icon}</span>
          <span className="hidden sm:inline">{roleMeta.label}</span>
        </div>

        {/* Right: bell + cart + user */}
        <div className="flex items-center gap-2">
          ...
          {state.currentRole === "customer" && !isGuest && (
            <button
              onClick={onCartClick}
              className="relative flex items-center gap-2 px-3 py-2 rounded-xl text-sm font-bold text-white transition-all active:scale-95"
          ...
          {isGuest && (
            <div className="flex items-center gap-2">
              ...
              <button
                ...
                style={{ background: "rgba(188,93,93,0.8)" }}
              >
                Login to Order
              </button>
            </div>
          ...
          {user && !isGuest && (
            <div className="relative">
              ...
              >
                ...
                <span className="hidden sm:block text-xs font-semibold text-white/80 max-w-[80px] truncate">
                  {displayName}
                </span>
              ...
              {userOpen && (
                <>
                  ...
                  <div
                    className="absolute top-full right-0 mt-2 rounded-2xl shadow-2xl p-2 min-w-[180px] z-50"
